...
- Android encrypts tokens, cached records and queued payloads with AES-GCM using an Android Keystore key. Backups and transfers are excluded. Corrupt encrypted data is preserved for explicit recovery instead of silently discarded. App screenshots are blocked for private worker screens.
